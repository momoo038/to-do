...
projectItems.forEach((itemData) => {
  ...
  const removeIcon = domUtils.createImage("images/remove.svg", "Remove card", {
    classes: ["trash-can-icon", "remove-card-hidden"],
  });

  const checkProjectCompletion = () => {
    const allTasksCompleted = itemData.tasks && itemData.tasks.length > 0 && itemData.tasks.every(task => task.isCompleted);
  ...
  if (itemData.tasks && itemData.tasks.length > 0) {
    ...
    itemData.tasks.forEach((taskData) => {
      ...
      mainInfoDiv.appendChild(checkbox);
      mainInfoDiv.appendChild(taskText);
      
      mainInfoDiv.addEventListener("click", () => {
